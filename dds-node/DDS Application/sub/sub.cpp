...
extern "C" int subscriber_main(int domainId, int sample_count)
{
	...
	Topic *topic1 = NULL;
	Topic *topic2 = NULL;
	UserDataTypeListener *reader_listener1 = NULL;
	UserDataTypeListener *reader_listener2 = NULL;
	...
	/* 5. 创建一个监听器 */
	reader_listener1 = new UserDataTypeListener();
	...
	reader1 = subscriber->create_datareader(
		...
		reader_listener1/* listener */, STATUS_MASK_ALL);
	if (reader1 == NULL) {
		fprintf(stderr, "create_datareader error\n");
		subscriber_shutdown(participant);
		delete reader_listener1;
		return -1;
	}

	topic2 = participant->create_topic(
		"topic_b"/* 主题名，应与发布者主题名一致 */,
		type_name, TOPIC_QOS_DEFAULT/* 默认QoS */, 
		NULL /* listener */, STATUS_MASK_NONE);
	if (topic2 == NULL) {
		fprintf(stderr, "create_topic error\n");
	...
	/* 5. 创建一个监听器 */
	reader_listener2 = new UserDataTypeListener();
	...
	reader2 = subscriber->create_datareader(
		...
		reader_listener2/* listener */, STATUS_MASK_ALL);
	if (reader2 == NULL) {
		fprintf(stderr, "create_datareader error\n");
		subscriber_shutdown(participant);
