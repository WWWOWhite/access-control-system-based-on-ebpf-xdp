...
extern "C" int publisher_main(int domainId, int sample_count)
{
	...
	for (count = 0; (sample_count == 0) || (count < sample_count); ++count) {
		...
		instance->x =  count;
		instance->y = 1 + count;
		strcpy(instance->color, "topic_a");
		GUID_t writerGuid = writer->getGuid();
        	char command[256];  // 用于存储完整的命令，假设最大长度不会超过 256
        	char hexString[33];
        	char *ptr = hexString;

        	for (int i = 0; i < 12; i++) {
               	 ptr += sprintf(ptr, "%02x", writerGuid.guidPrefix[i]);
        	}

        	for (int i = 0; i < 3; i++) {
                	ptr += sprintf(ptr, "%02x", writerGuid.entityId.entityKey[i]);

        	}	
        	ptr += sprintf(ptr, "%02x", writerGuid.entityId.entityKind);

        	*ptr = '\0';
       		snprintf(command, sizeof(command), "./send_post.sh %s %s", hexString, topic_name);
        	std::cout << "Command after building: " << command << std::endl;
